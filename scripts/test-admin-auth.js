...
try {
  ...
  console.log('\n[4/9] Testing Authorization & RBAC Boundaries...');
  {
    ...
    // 4.8: CRITICAL: Correct admin email + WRONG Firebase UID -> HTTP 403 and isAuthorizedAdmin === false
    const wrongUidAdmin = {
      uid: 'attacker_impersonating_uid',
      email: 'rajshreeakm@gmail.com',
      role: 'admin',
      isAdmin: true,
      isAnonymous: false,
    ...
    const mockResWrongUid = createMockRes();
    let nextCalledWrongUid = false;
    requireAdmin({ user: wrongUidAdmin }, mockResWrongUid, () => {
      nextCalledWrongUid = true;
    });
    assert.strictEqual(nextCalledWrongUid, false, 'Admin with wrong UID must NOT pass requireAdmin');
    assert.strictEqual(mockResWrongUid.getStatusCode(), 403, 'Admin with wrong UID must receive HTTP 403');
    assert.strictEqual(isAuthorizedAdmin(wrongUidAdmin, wrongUidAdmin), false, 'isAuthorizedAdmin must return false on wrong UID');

    // 4.9: CRITICAL: Correct admin email + unconfigured ADMIN_FIREBASE_UID in production -> FAIL CLOSED
    const prevEnvUid = process.env.ADMIN_FIREBASE_UID;
    const prevEnvNode = process.env.NODE_ENV;
    ...
    try {
      ...
      const mockResUnconfigured = createMockRes();
      let nextCalledUnconf = false;
      requireAdmin({ user: validAdmin }, mockResUnconfigured, () => {
        nextCalledUnconf = true;
      });
      assert.strictEqual(nextCalledUnconf, false, 'Unconfigured ADMIN_FIREBASE_UID in prod must fail closed');
      assert.strictEqual(mockResUnconfigured.getStatusCode(), 403, 'Unconfigured admin UID must yield HTTP 403');
      assert.strictEqual(isAuthorizedAdmin(validAdmin, validAdmin), false, 'isAuthorizedAdmin must fail closed');
    } finally {
      process.env.ADMIN_FIREBASE_UID = prevEnvUid;
      process.env.NODE_ENV = prevEnvNode;
      process.env.ENGINEERVERSE_TEST_RUNNER = prevTestRunner;
    }

    // 4.10: Correct admin email + correct UID + unverified email -> HTTP 403
    const unverifiedAdmin = {
      ...validAdmin,
      emailVerified: false,
    };
    const mockResUnverifiedAdmin = createMockRes();
    let nextCalledUnverifiedAdmin = false;
    requireAdmin({ user: unverifiedAdmin }, mockResUnverifiedAdmin, () => {
      nextCalledUnverifiedAdmin = true;
    });
    assert.strictEqual(nextCalledUnverifiedAdmin, false, 'Unverified admin email must not pass requireAdmin');
    assert.strictEqual(mockResUnverifiedAdmin.getStatusCode(), 403, 'Unverified admin email must receive HTTP 403');
    assert.strictEqual(isAuthorizedAdmin(unverifiedAdmin, unverifiedAdmin), false);
    ...
    console.log('   ✓ Impersonation defense: Correct admin email + WRONG Firebase UID rejected with 403');
    console.log('   ✓ Fail-closed defense: Missing ADMIN_FIREBASE_UID strictly denies admin authorization');
    console.log('   ✓ Verified identity defense: Unverified admin email strictly rejected with 403');
  }
